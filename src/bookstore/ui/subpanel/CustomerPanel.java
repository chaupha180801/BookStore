...
public class CustomerPanel extends javax.swing.JPanel {
    ...
    public void renderTable(ArrayList<Customer> customers){
        String[] columnName = {"Mã khách hàng", "Tên khách hàng", "Loại khách hàng", "Địa chỉ",
                                "SĐT", "Tổng nợ"};
        Object[][] rows = new Object[customers.size()][6];
        
        for(int i = 0; i < customers.size(); i ++){
            rows[i][0] = customers.get(i).getId();
            rows[i][1] = customers.get(i).getName();
            rows[i][2] = customers.get(i).getTypeOfCustomer();
            rows[i][3] = customers.get(i).getAddress();
            rows[i][4] = customers.get(i).getPhoneNumber();
            rows[i][5] = customers.get(i).getDebt();
        }
        DefaultTableModel model = new DefaultTableModel(rows, columnName);
        this.table.setModel(model);
    }
    public Customer getSelectedRowInTable(){
        ...
        int typeOfCustomer = Integer.valueOf(this.table.getValueAt(row, 2).toString());
        String address = this.table.getValueAt(row, 3).toString();
        String phoneNumber = this.table.getValueAt(row, 4).toString();
        int debt = Integer.valueOf(this.table.getValueAt(row, 5).toString());       
        return new Customer(id, name, typeOfCustomer, address, phoneNumber, debt);
    }
    public void reloadTable(){
        try{
            ArrayList<Customer> customers = Customer.selectAll();
    ...
    private void initComponents() {
        ...
        titleLabel.setText("QUẢN LÝ THÔNG TIN KHÁCH HÀNG");
        ...
        table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã khách hàng", "Tên khách hàng", "Loại khách hàng", "Đia chỉ", "SĐT", "Tổng nợ"
            }
        ));
    ...
    private void searchButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchButtonActionPerformed
        ...
        try{
            ...
            ArrayList<Customer> customers = Customer.findByName(input,false);
            this.renderTable(customers);
        }catch(DatabasePermissonErrorException ex){
    ...
    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        // TODO add your handling code here:
        Customer customer = this.getSelectedRowInTable();

        if (customer == null)
        return;

        AddCustomer addCustomer = new AddCustomer(this, customer);
        addCustomer.show();
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        try{
            ...
            int option = JOptionPane.showConfirmDialog(
                null,
                "Bạn có chắc muốn xóa " + customer.getName() + " không? ",
                "Xóa",
