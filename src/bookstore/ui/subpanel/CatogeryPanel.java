...
public class CatogeryPanel extends javax.swing.JPanel {   
    ...
    public void renderTable(ArrayList<Category> types){
        String[] columnName = {"Mã thể loại","Tên thể loại","Diễn giải"};
        Object[][] rows = new Object[types.size()][3];
        
        for(int i = 0; i < types.size(); i ++){
            rows[i][0] = types.get(i).getId();
            rows[i][1] = types.get(i).getName();
            rows[i][2] = types.get(i).getExplain();
        }
        DefaultTableModel model = new DefaultTableModel(rows, columnName);
        this.table.setModel(model);
    }
    public Category getSelectedRowInTable(){
        ...
        String explain = this.table.getValueAt(row,2).toString();
        return new Category(id, name, explain);
    }
    public void reloadTable(){
        try{
            ArrayList<Category> types = Category.selectAll();
    ...
    private void initComponents() {
        ...
        titleLabel.setText("QUẢN LÝ THÔNG TIN THỂ LOẠI");
        ...
        table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã thể loại", "Tên thể loại", "Diển giải"
            }
        ) {
    ...
    private void searchButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchButtonActionPerformed
        ...
        try{
            ...
            ArrayList<Category> types = Category.findByName(input,false);
            this.renderTable(types);
        }catch(DatabasePermissonErrorException ex){
    ...
    private void addButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addButtonActionPerformed
        // TODO add your handling code here:
        AddCatogery addCatogery = new AddCatogery(this);
        addCatogery.show();
    }//GEN-LAST:event_addButtonActionPerformed

    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        // TODO add your handling code here:
        Category type = this.getSelectedRowInTable();

        if (type == null)
            return;

        AddCatogery addCatogery = new AddCatogery(this, type);
        addCatogery.show();
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        try{
            ...
            int option = JOptionPane.showConfirmDialog(
                null,
                "Bạn có chắc muốn xóa " + type.getName() + " không? ",
                "Xóa",
                JOptionPane.YES_NO_OPTION
            );

            if(option == JOptionPane.YES_OPTION){
                if(Category.delete(type)){
