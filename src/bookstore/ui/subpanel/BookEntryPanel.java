...
import bookstore.database.tables.BookEntry;
...
public class BookEntryPanel extends javax.swing.JPanel {

    /**
     * Creates new form BookEntryPanel
     */
    ...
    public void renderTable(ArrayList<BookEntry> bookEntrys){
        String[] columnName = {"Mã phiếu nhập", "Ngày nhập sách", "Tổng tiền",};
        Object[][] rows = new Object[bookEntrys.size()][3];
        
        for(int i = 0; i < bookEntrys.size(); i ++){
            rows[i][0] = bookEntrys.get(i).getId();
            rows[i][1] = bookEntrys.get(i).getDate();
            rows[i][2] = bookEntrys.get(i).getTotalPrice();          
    ...
    public BookEntry getSelectedRowInTable(){
        try{
            ...
            java.sql.Date sqlngnhap = new java.sql.Date(startdate.getTime());
            int totalPrice = Integer.valueOf(this.table.getValueAt(row,2).toString());
            return new BookEntry(id, sqlngnhap, totalPrice);
    ...
    public void reloadTable(){
        try{
            ArrayList<BookEntry> bookEntry = BookEntry.selectAll();
            this.renderTable(bookEntry);
        }catch(DatabasePermissonErrorException ex){
    ...
    private void initComponents() {
        ...
        titleLabel.setText("QUẢN LÝ THÔNG TIN PHIẾU NHẬP SÁCH");
    ...
    private void searchButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchButtonActionPerformed
        ...
        try{
            ...
            BookEntry bookEntry = BookEntry.findById(input);
            ArrayList<BookEntry> bookEntrys = new ArrayList<>();
            bookEntrys.add(bookEntry);
            this.renderTable(bookEntrys);
        }catch(DatabasePermissonErrorException ex){
    ...
    private void addButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addButtonActionPerformed
         try{
            BookEntry bookEntry = BookEntry.insert(new Date());
            AddBookEntry addBookEntry = new AddBookEntry(this, bookEntry);
            addBookEntry.show();
        }catch(Exception e){
    ...
    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        // TODO add your handling code here:
        BookEntry bookEntry = this.getSelectedRowInTable();
        if (bookEntry == null)
            return;
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        try{
            ...
            int option = JOptionPane.showConfirmDialog(
                null,
                "Bạn có chắc muốn xóa " + bookEntry.getId() + " không? ",
                "Xóa",
                JOptionPane.YES_NO_OPTION
            );
            if(option == JOptionPane.YES_OPTION){
                if(BookEntry.delete(bookEntry.getId())){
